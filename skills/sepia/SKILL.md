...
**Experimental — composing with a voice skill:** when the user says a voice or style skill is stacked with sepia (a minimalism method, a brand voice, a persona guide), add `references/voice-skills.md` on top of the normal route. Opt-in only: never assume a voice skill is in play, and never inject one. Built-in profile bodies under `references/voices/` load only when the user opts in; the exact opt-in phrases are listed in `references/voice-skills.md` (currently `apply the Hemingway voice`, and for professional routes `apply the Taiwan journalism voice` / 「套用台灣深度報導 voice」, optionally followed by a shape name), and a request that contains one of them is an opt-in on every route that profile supports.
