...
**Closing line.** Write, recreate and refactor stage 2 end with one line outside the prose: `Persona applied: <name> — moves: <3–8 move names>`. When fewer than three moves have facts, write and recreate print `Persona applied: <name> — moves: <names> — fewer: facts`; when refactor stage 2 applied fewer than three moves as defect fixes, it prints `Persona applied: <name> — moves: <names> — fewer: defects`. A move is never applied to reach a count. On a route the persona does not declare, those three operations print `Persona applied: none — <name> declares routes: <routes>`. Review and refactor stage 1 print their normal report and no `Persona applied:` line; a route mismatch goes into their one-line notice. The line is distinct from a venue profile's `Voice applied:` line and from the registry's `Voice fit:` line; a piece under both a venue voice and a persona ends with both lines, persona last.
