...
After A — 場景導入 (moves: open on a person; gesture after the quotation; scene in, institution out; return to the person):

> 3 月 4 日下午 2 點 05 分，北部一間區域醫院的急診，值班護理師把三張紙本檢傷單夾到白板上，繼續分級。「電腦轉圈，我就先用紙。」她把筆插回口袋。
>
> 電腦轉圈的原因在急診之外。院內系統的重試佇列沒有設 jitter，一批失敗的請求在 200 毫秒內重送了 412 次；平時一小時裡，每分鐘大約只有 3 次。下游服務互相限流，誰都進不去。
>
> 2 點 41 分，值班平台工程師找到原因；2 點 47 分，他把 `RETRY_JITTER=full` 設上。他說，這個佇列的預設值從來沒有人檢視過。3 月 6 日，所有佇列的預設值都改了。
>
> 白板上的三張紙本檢傷單，那天下午沒有再增加。
>
> Voice applied: tw-journalism/場景導入 — moves: open on a person, gesture after quotation, scene in institution out, return to the person

| Change | Move | Check |
|---|---|---|
| 「本文將探討…」 deleted; a time, a place, a person and an action open the piece | Open on a person | `discourse-pass.md` §4; `journalism.md` tells row 1 |
| 「無奈地表示：“…”」 → the quotation, then 「她把筆插回口袋」 | Gesture after quotation | zh.md §2 manner adverb; `journalism.md` rule 3; quote marks 「」 |
| 「——…——」 paired insertion → a plain sentence; 412, 200 ms, baseline 3 per minute added | Scene in, institution out | check 5; zh.md §1b paired dash row |
| 「非常」「迅速地」「成功地」 deleted | (register default) | zh.md §1b, §2 |
| 「總而言之…重要性…謹慎」 → the whiteboard at a later moment | Return to the person | check 7 |

Known cost of After A: check 2 will report the whiteboard sentences as detail; `journalism.md` rule 8 says they are information for this reader, and the report should list them as the voice's cost, not a defect. The last sentence carries no outcome the reader still needs, because the outcome sits in paragraph three.
...
To be filled before merge from the blind review specified in the PR's acceptance: After A reviewed twice by a fresh executor on the professional route, once with the voice declared and once without; recorded per run: whether the uniformity row fired, the check 2 and check 7 advisories, and that no `Voice fit:` line was printed.
