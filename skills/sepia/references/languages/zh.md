...
| Parenthetical notes: 甲式 （　） for annotation, including a foreign original or dates after a name; 乙式 ──　── when the sentence flows through the supplement | `MOE-PUNCT-2008` 夾注號 (official standard) | Fix half-width ( ) around Chinese text to full-width （）; keep a foreign original in parentheses on first mention |
| Numerals: Arabic for counts, statistics, measures, dates, clock times, ordinals, codes; Chinese numerals for idioms, set phrases and proper names (三讀、九二一、一線) | `EY-NUMERALS-2004` (official administrative standard for government documents; newspapers add house rules on percentages and era years) | Venue-gated: on a Taiwanese news venue, fix a spelled-out count or measure of eleven or more (「一千一百小時」「三十四位」) to Arabic digits, changing numeral form only, when the venue's own samples write such counts in digits or when no samples are available; samples that spell them out win, and the row takes no action. Eleven is the lowest value on which every consulted guide agrees (AP and BBC spell out 1–9, the Economist 1–10, one Taiwanese NGO 一至十); counts of ten and below (「三位」「十位」) are left as written and the venue decides. Idioms and names stay in Chinese. Do not mix 「百分之」 and 「%」 in one piece (consensus, guides §五 7). Measured usage agrees (§1c) |
| Titles and names: on first mention the full title precedes the full name (行政院長某某某) | One first-tier house rule, an older newspaper 通則 (guides §三; not a consensus, guides §四 10) | Venue-gated: fix a title placed after the name (某某某行政院長) only when the venue's own samples put the title first; with no samples, or samples that use the other order, no action |
...
| Work titles: 《》 for books, films, albums, publications, documents; 〈〉 for articles, chapters, songs; a piece's own headline takes no 書名號 | `MOE-PUNCT-2008` 書名號 乙式 (official standard) | Fix “ ” or 「」 used around a work title; keep 「」 for terms and speech |
| Attribution verb: 「說」 is the default; 「表示」「指出」「強調」「坦言」 carry a judgment and are used when that judgment is meant | Consensus (guides §五 3) | Report a run of judgment verbs where 「說」 would do; do not rewrite quotations to change the verb inside them. sepia does not restore 「表示」 (guides §六 3, inference) |
| Questions: a question mark follows an interrogative sentence; a rhetorical question does not replace a verified fact | `MOE-PUNCT-2008` 問號 (official standard); consensus (guides §五 9) | Report a rhetorical question standing where a fact was needed. Leave paragraph-end questions alone (§1b: register-normal) |
| Taiwan lexicon on a Taiwanese venue (already in §5) | venue-corpus guardrail | Unchanged: fix 視頻、軟件、質量 only when the venue is Taiwanese and the author's samples do not use them |
