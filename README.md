...
The contract in short: sepia's architecture decisions come first. The voice's moves are applied selectively (3–5 signature moves per piece, fewer when a sparse shape or the facts offer fewer; formula endings deliberately broken sometimes). Review reports the voice's known costs instead of fixing them away, while uniformity findings keep full strength: a voice does not excuse a metronome. On professional routes the venue still sets the register. Direct conflicts come back to you. The interface is grounded in one blind review experiment on a strict-minimalism specimen — a worked example, not measured evidence. Two built-in profiles ship under `references/voices/`: Hemingway (iceberg omission for fiction, the Kansas City Star rules for professional prose, each move traced to its source) and, for professional routes only, a Taiwan long-form journalism voice (nine narrative shapes with their moves, drawn from the private human-side journalism measurement and its close reading; opt-in phrase "apply the Taiwan journalism voice"). Both built-in profiles follow the same rule. A third profile kind, the persona (one writer's style with declared override rights), has a template at `references/voices/PERSONA-TEMPLATE.md` and a validator at `scripts/check_persona.py`, with one built-in persona, the project's own companion voice, written from its maintainer's voice specification with her own exemplars, professional routes only (opt-in phrase "apply persona Nyaneko" or 「套用 persona Nyaneko」). On fiction, a review only reports when your text's recorded findings fit the Hemingway profile; it loads nothing. Asking for strong de-AI on a story counts as opting in, and sepia then says which profile it is applying and how to decline. `/sepia-hemingway` is the direct entry.

### Writing with a persona: a short how-to

sepia's rules remove tells. They do not supply warmth; that comes from the writer knowing who is speaking. One case from this project's own release, observed rather than measured: the v0.12.0 Threads post was drafted twice by the same model from the same release notes. The first prompt handed it a skeleton (version line, bullets, fixes, engineering note, update line) and got a form filled in. The second prompt handed it an identity, "I maintain this project and just shipped v0.12.0; talk to my followers in my own voice; how you organise it is yours", plus the notes as the only fact source and the platform's limits, and got the post the maintainer kept. A sepia review of that post then found its tells clustered at the opening and the close (a journey introduction, a maxim ending, a few absolutes), and a refactor took them out without touching the middle.

So the order that worked: identity first, facts separately, constraints stated as platform limits rather than structure, then sepia as the check.
...
The style pass checks the *spread* of sentence lengths, the one syntactic measure on which every study that measured it agrees (human text varies more within a passage, in English and in Chinese); mean sentence length, punctuation counts, and paragraph length are not treated as signals because the measured directions contradict each other. Chinese text loads `references/languages/zh.md`, a calibration built on the human-vs-machine Chinese corpus HC3 (2023), a private human-side measurement of Traditional Chinese journalism (about two thousand articles from one unnamed Taiwanese publication over about ten years; corpus not distributed), and a small machine-side contrast group for that register (three models, one shared base prompt with a one-line variant for one model, 119 synthetic pieces), with the limits of all three stated in the file, and a short normative section of Taiwan writing conventions taken from public standards and first-tier consensus; evidence and numbers are in `research/rhythm-syntax.md`, and the journalism digest is `research/zh-news-corpus.md`.
