...
這是 [Agent Skill](https://agentskills.io/specification)：任何支援這個標準的 agent 都能載入；[Skills CLI](https://skills.sh)（支援 77+ 家 agent）一行指令就能裝。Claude Code、Codex、Grok Build、Antigravity 與 QwenPaw 另有原生 plugin 打包。全平台共用唯一正典 `SKILL.md`，不另開平台分支。四種操作：**write**、**review**（只診斷）、**refactor**（最小修改）、**recreate**（整篇重寫）。
...
約定摘要：sepia 的架構決策先行，聲音技法選擇性套用（每篇挑 3–5 招招牌技法，敘事型稀疏或事實不夠時更少；招牌結尾公式偶爾故意打破）。review 只回報聲音的已知代價、不代修，但均勻性 finding 不打折：聲音不能豁免節拍器。專業路由上 venue 仍定語域，衝突交回給你決定。這個介面依據極簡規格樣本的盲審實驗，屬單一案例，非量測證據。`references/voices/` 下附兩個內建 profile：海明威（小說用冰山省略、專業文體用堪薩斯市星報規則，每招都標出處），以及只在專業路線上用的台灣深度報導 voice（九種敘事型各自的手法，來自那份私下量測的繁中新聞語料與細讀；opt-in 片語「套用台灣深度報導 voice」）。兩個內建 profile 都照同一條規則。第三種 profile 是 persona（單一寫作者的風格，帶宣告式覆蓋權），模板在 `references/voices/PERSONA-TEMPLATE.md`，驗證腳本在 `scripts/check_persona.py`，內建一個 persona：專案自己的夥伴聲線，依維護者寫的語音規格撰寫並附她本人的示範句，僅限專業路由（opt-in 片語「套用 persona Nyaneko」或 `apply persona Nyaneko`）。小說路線上，review 發現你的文本記錄到的 findings 符合海明威 profile 時只會提示，不載入任何東西；你說要「強力去 AI 味」才算 opt-in，sepia 會講明正在套用哪個 profile、怎麼取消。`/sepia-hemingway` 是直接入口。

### 用 persona 寫東西：一段簡短教學

sepia 的規則負責拿掉 AI 味，不負責給溫度。溫度來自寫的人知道自己是誰在對誰說話。這個專案自己的一個案例，是觀察不是量測：v0.12.0 的 Threads 貼文用同一個模型、同一份 release notes 寫了兩次。第一次的提示給了骨架（版本行、條列、修正、工程備註、更新方式），出來是照表填格。第二次的提示給的是身分，「我是這個專案的維護者，剛發了 v0.12.0，用我自己的口吻講給追蹤者聽，怎麼組織由你決定」，加上 notes 當唯一事實來源和平台限制，出來的是維護者留下來用的那篇。之後用 sepia review 掃那篇，tell 集中在開頭與收尾（歷程鋪陳、格言式結尾、幾個絕對化的詞），refactor 拿掉它們，中間沒動。

有效的順序是：先給身分，事實另附，限制寫成平台限制而不是結構，最後用 sepia 當檢查。
...
persona profile 就是把這個身分寫下來一次，之後每篇都從它出發：她對讀者是誰、第一句做什麼、溫度掛在什麼事實上、絕不做什麼，以及幾篇她自己聲音的範文。模板在 `references/voices/PERSONA-TEMPLATE.md`，內建的 `nyaneko.md` 是一份完整範例，`python3 scripts/check_persona.py <file>` 檢查散文外圍的介面。鼓勵你寫自己的一套；範文承載的東西比描述多。

## 句長節奏與中文校準

style pass 會檢查句長的變化幅度，這是有量到它的研究裡唯一方向一致的句法量測（人類文本在同一段內變化較大，英文與中文皆然）；句長平均值、標點計數、段落長度不當訊號，因為量測方向互相矛盾。中文文本會載入 `references/languages/zh.md`，校準建立在人機對照的中文語料 HC3（2023）、一份人類側的繁中新聞量測（一家未具名台灣出版方，約兩千篇，橫跨約十年，語料未散布），以及同一語域的小型機器側對照組（三個模型、同一份基礎提示、其中一個模型多一行變體、119 篇合成文），三者的限制都寫在檔案裡，另有一節取自公開規範與一級共識的台灣寫作慣例；證據與數字見 `research/rhythm-syntax.md`，新聞量測的摘要在 `research/zh-news-corpus.md`。
...
[Skills CLI](https://skills.sh) 支援的 agent 都能裝：Cursor、Cline、Windsurf、Copilot、OpenCode、goose 等；安裝時會問你要裝到哪幾家。五大平台以外的執行行為我們沒有實測過；skill 本體是 Agent Skills 標準下的純 markdown，你的 agent 若吃不動歡迎開 issue。

下面五個平台另有原生 plugin installer，各自實機裝過（QwenPaw 由貢獻者裝，見該節）。驗證範圍是裝得完、sepia 入口出現；裝好之後入口的行為沒有逐平台實測。
...
由貢獻者在 QwenPaw 2.2.1 上實機驗證（#250，維護者未自行重跑）：裝得完、`/sepia` 有路由，package 裡的 `skills` 符號連結會被 `shutil.copytree` 展開成實體目錄。
