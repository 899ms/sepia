...
有效的順序是：先給身分，事實另附，限制只寫平台限制、結構留給模型，最後用 sepia 當檢查。
...
persona profile 就是把這個身分寫下來一次，之後每篇都從它出發：她跟讀者是什麼關係、第一句做什麼、溫度掛在什麼事實上、絕不做什麼，以及幾篇她自己聲音的範文。模板在 `references/voices/PERSONA-TEMPLATE.md`，內建的 `references/voices/personas/nyaneko.md` 是一份完整範例，`python3 scripts/check_persona.py <file>` 檢查散文外圍的介面。鼓勵你寫自己的一套；範文承載的東西比描述多。
