...
简单说，sepia 的架构决策先行，声音手法的套用必须节制（每篇选取 3–5 种代表手法，偶尔有意打破公式化的收尾）。review 会报告这个声音已知的代价，不代改，但均匀性 finding 不打折：有了声音也不能写成节拍器。在专业写作路径上，语域仍由发布场合决定，一旦发生直接冲突就交由你定夺。这套接口源自针对一份极简主义样本的盲审实验，属单一案例，并非量测证据。`references/voices/` 内置了两套 profile：海明威（小说用冰山式省略，专业文体遵循堪萨斯城星报规则，每项手法都标明出处），以及仅用于专业路径的台湾深度报道 voice（九种叙事形态各自的手法，来自那份私下测量的繁体中文新闻语料及其细读；opt-in 短语「套用台灣深度報導 voice」）。内置 profile 也遵守同一条规则。小说路线上，review 发现你的文本记录到的 findings 符合海明威 profile 时只会提示，不加载任何东西；要求强力去除故事里的 AI 味才算 opt-in，sepia 会说明正在套用哪个 profile、怎么关闭。`/sepia-hemingway` 为直达入口。
