...
persona profile 就是把这个身份写下来一次，之后每篇都从它出发：她和读者是什么关系、第一句做什么、温度挂在什么事实上、绝不做什么，以及几篇她自己声音的范文。模板位于 `references/voices/PERSONA-TEMPLATE.md`，内置的 `references/voices/personas/nyaneko.md` 是一份完整示例，接口校验脚本负责检查 profile 结构：
