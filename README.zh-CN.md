...
专业文档暴露破绽的方式不同，而结构层的结论在这里同样成立：2026 年的一项研究把 StoryScope 复制到商业博客文章上，2,250 篇真人文对上 11,250 篇 AI 镜像，仅用结构特征就以 98.0 macro-F1 区分两者，AI 那一侧被描述为「整齐、自我预告」（arXiv:2609.15369，预印本，特征由 LLM 评分；它测的是检测，不是编辑）。[`research/`](research/) 里梳理的研究指出的问题是：没有信息量的填充文字、需要判断时闪烁其词、聊天机器人的残留语气、无视具体场合的语域、像一个模子印出来的排版。每类文档都在一份共用检查清单之上，各配一份精简的规则文件。
...
简单说，sepia 的架构决策先行，声音手法的套用必须节制（每篇选取 3–5 种代表手法，叙事型稀疏或事实不足时更少；偶尔有意打破公式化的收尾）。review 会报告这个声音已知的代价，不代改，但均匀性 finding 不打折：有了声音也不能写成节拍器。在专业写作路径上，语域仍由发布场合决定，一旦发生直接冲突就交由你定夺。这套接口源自针对一份极简主义样本的盲审实验，属单一案例，并非量测证据。`references/voices/` 内置了两套 profile：海明威（小说用冰山式省略，专业文体遵循堪萨斯城星报规则，每项手法都标明出处），以及仅用于专业路径的台湾深度报道 voice（九种叙事形态各自的手法，来自那份私下测量的繁体中文新闻语料及其细读；opt-in 短语「套用台灣深度報導 voice」）。两套内置 profile 都遵守同一条规则。第三种 profile 是 persona（单一写作者的风格，带声明式覆盖权），模板在 `references/voices/PERSONA-TEMPLATE.md`，校验脚本在 `scripts/check_persona.py`，暂无内置 persona。小说路线上，review 发现你的文本记录到的 findings 符合海明威 profile 时只会提示，不加载任何东西；要求强力去除故事里的 AI 味才算 opt-in，sepia 会说明正在套用哪个 profile、怎么关闭。`/sepia-hemingway` 为直达入口。

## 句长节奏与中文校准

style pass 重点检查句长的*变化幅度*。这是量测过它的研究里唯一方向一致的句法指标（人类文本在同一段内变化更大，英文与中文皆然）；平均句长、标点数量、段落长度不当信号，因为各项量测的方向相互矛盾。中文文本会加载 `references/languages/zh.md`，这份校准基于人机对照的中文语料库 HC3（2023）、一份人类侧的繁体中文新闻测量（一家未具名台湾出版方，约两千篇，跨约十年，语料未公开），以及同一语体的小型机器侧对照组（三个模型、同一份基础提示、其中一个模型多一行变体、119 篇合成文），三者的局限均已在文件内说明，另有一节取自公开规范与一级共识的台湾写作惯例。具体证据与数据见 `research/rhythm-syntax.md`，新闻测量的摘要见 `research/zh-news-corpus.md`。
